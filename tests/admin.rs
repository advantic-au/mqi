...
fn list_local_queues() -> Result<(), Box<dyn std::error::Error>> {
    let admin_bag = Bag::new(BagType::Admin, sys::MQCBO_NONE)?;
    admin_bag.add(sys::MQCA_Q_NAME, "*")?;
    admin_bag.add(sys::MQIA_Q_TYPE, sys::MQQT_ALL)?;
    ...
    let execute_result = admin_bag
        .execute(conn.handle(), sys::MQCMD_INQUIRE_Q, None, None, None)
        .warn_as_error()?;

    for bag in execute_result
        .try_iter::<Bag<_, _>>(sys::MQHA_BAG_HANDLE)?
        .flatten()
